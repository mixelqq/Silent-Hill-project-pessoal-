...
app.get("/posts", (req, res) => {
    ...
    db.all(
        ...
        "SELECT * FROM posts ORDER BY fixado DESC, id DESC",
...
app.post("/criar-post", (req, res) => {
    ...
    db.run(
        ...
        "INSERT INTO posts (titulo, mensagem, autor) VALUES (?, ?, ?)",
...
app.delete("/deletar/:id", (req, res) => {
    ...
    if (!req.session.usuario) {
        ...
        return res.send("Login necessário");
    }

    if (req.session.usuario.role !== "admin") {

        return res.send("Apenas admin");
    }
...
app.post("/fixar/:id", (req, res) => {
    ...
    db.run(
        ...
        "UPDATE posts SET fixado = 1 WHERE id = ?",

        [req.params.id],

        () => {

            res.send("Post fixado");
        }
    );
});
...
app.post("/comentar/:id", (req, res) => {
    ...
    db.run(
        ...
        "INSERT INTO comentarios (post_id, autor, comentario) VALUES (?, ?, ?)",
...
app.post("/banir", (req, res) => {
    ...
    db.run(
        ...
        "UPDATE usuarios SET banido = 1 WHERE email = ?",

        [req.body.email],

        () => {

            res.send("Usuário banido");
        }
    );
});
